...
import SiteVisitColumnModifier from '@/components/dashboard/sales/SiteVisitColumnModifier';

const VisitStatCard = ({ label, value, trend, icon, bgGradient, borderTopColor, iconBg }: any) => (
  <div 
...
const SiteVisitPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState('All Visit');
  ...
  const visits = [
    { id: '#13123', name: 'Rahul Sharma', contact: '******2413', requirement: '1 BHK, 2 BHK', budget: '₹90L - ₹1.5 Cr', status: 'Warm', source: 'Marketing' },
    { id: '#13123', name: 'Rahul Sharma', contact: '******2413', requirement: '1 BHK, 2 BHK', budget: '₹90L - ₹1.5 Cr', status: 'Cold', source: 'Marketing' },
    { id: '#13123', name: 'Rahul Sharma', contact: '******2413', requirement: '1 BHK, 2 BHK', budget: '₹90L - ₹1.5 Cr', status: 'Lost', source: 'Marketing' },
    { id: '#13123', name: 'Rahul Sharma', contact: '******2413', requirement: '1 BHK, 2 BHK', budget: '₹90L - ₹1.5 Cr', status: 'Ready To Revisit', source: 'Marketing' },
    { id: '#13123', name: 'Rahul Sharma', contact: '******2413', requirement: '1 BHK, 2 BHK', budget: '₹90L - ₹1.5 Cr', status: 'Hot', source: 'Marketing' },
  ];
      ...
      <div className="flex h-[calc(100vh-64px)] overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 scroll-smooth">
          ...
          <div className="bg-[#EAE2F1] -mx-6 px-6 py-2 mb-6 flex items-center gap-2">
            {['All Visit', 'Report/Stats'].map(tab => (
              <button
                key={tab}
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {['Marketing', 'Chembur', 'Vikhroli', 'Ghatkopar', '90 lacs - 1 Cr'].map(filter => (
               <div key={filter} className="flex items-center gap-2 bg-[#F1F5F9] border border-gray-100 px-4 py-2 rounded-xl text-[10px] font-black text-[#371D45] uppercase tracking-widest shadow-sm">
                  {filter}
                  <X className="w-3.5 h-3.5 text-gray-400 hover:text-red-400 cursor-pointer transition-colors" />
               </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-[32px] shadow-sm border border-gray-100 p-6 relative">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-4">
                <h2 className="text-3xl font-black text-[#1a1a1a] tracking-tight">Site Visits</h2>
              </div>
              ...
              <table className="w-full border-separate border-spacing-0">
                <thead>
                  <tr className="bg-[#FBF2FF]/80 text-[#4A1D59] text-[11px] font-black uppercase tracking-widest border-b border-purple-100">
                    ...
                    <th className="py-3 px-6 text-center border-b border-purple-50">Source</th>
                  ...
                  {[...Array(12)].map((_, i) => {
                    ...
                    return (
                      <tr key={i} className="group hover:bg-slate-50/50 transition-all">
                        <td className="py-3 px-6 text-[11px] font-bold text-gray-400">{visit.id}</td>
                        ...
                        <td className="py-3 px-6 text-xs font-bold text-gray-600">{visit.contact}</td>
                        <td className="py-3 px-6 text-xs font-bold text-[#371D45]/80">{visit.requirement}</td>
                        <td className="py-3 px-6 text-xs font-bold text-[#371D45]/80">{visit.budget}</td>
                        <td className="py-3 px-6">
                          <div className="flex justify-center">
                            <Badge className={`${getStatusColor(visit.status)} text-white px-5 py-1 rounded-[12px] text-[10px] font-black uppercase tracking-wider border-none shadow-md shadow-gray-200/50 w-[140px] justify-center whitespace-nowrap`}>
                              {visit.status}
                            </Badge>
                          </div>
                        </td>
                        <td className="py-3 px-6 text-center text-xs font-bold text-gray-500">{visit.source}</td>
            ...
            <div className="mt-8 flex items-center justify-between text-xs font-bold text-gray-400 px-6">
                <p>Showing 1 to 12 of 40 entries</p>
